        ...
        <div class="flex gap-x-5">
            ...
            <div class="absolute top-14 left-16 md:top-0 md:left-0 md:relative group cursor-pointer border-2 border-primary-500 hover:shadow-lg hover:bg-zinc-200 transition rounded-xl p-2.5">
                ...
                if (isset($_SESSION['user_sending'])) {
                    // کاربر لاگین
                    $cartCount = getCartTotalQuantity($_SESSION['user_sending']);
                } else {
                    // مهمان
                    $cartCount = getCartTotalQuantity();
                }
                ?>
                    <span id="countCart" class="absolute -right-2 -top-2 flex size-6 drop-shadow-lg items-center justify-center rounded-full bg-white text-sm font-semibold text-primary-500">
                    <?= $cartCount ?? '0' ?>
                </span>
                ...
                <div class="z-50 hidden group-hover:block absolute -left-16 sm:left-0 top-11 w-96 md:w-[400px] rounded-2xl border border-zinc-300 bg-white shadow-lg">
                    ...
                    <div class="h-72 md:h-64">
                        <ul id="listCart" class="main-scroll h-full space-y-2 divide-y divide-gray-100 overflow-y-auto p-5 pl-2">
                            <?php
                            //unset($_SESSION['cart']);
                            if (isset($_SESSION['user_sending'])) {
                            ...
                            $getOneProduct = getOneProduct($cart['product_id']);
                                ...
                                $thumbnailProductCart = !empty($mainImage) ? "public/images/products/{$mainImage}" : '';
                            ?>
                            <li id="product<?= $getOneProduct['id'] ?>">
                                <div class="flex gap-x-2 pt-5">
                                    <!-- Product -->
                                    <div class="relative min-w-fit">
                                        <a href="./single-product.html">
                                            <img alt="" class="h-[120px] w-[120px]" src="<?= $thumbnailProductCart ? "../../" . $thumbnailProductCart : '' ?>">
                                        </a>
                                    </div>
                                ...
                                <div class="flex justify-between px-6 w-full pb-2">
                                    <!-- color -->
                                    <?php if (!empty($cart['color']) && !empty($cart['titleColor'])): ?>
                                        <div class="flex gap-x-2 text-sm text-gray-600 items-center">
                                            <div class="w-4 h-4 rounded-full border" style="background-color: <?= htmlspecialchars($cart['color']) ?>;">
                                            </div>
                                            <?= htmlspecialchars($cart['titleColor']) ?>
                                        </div>
                                    <?php endif; ?>
                                    <!-- Price -->
                                    <div class="flex flex-col items-end">
                                        <?php if ($cart['discount'] < $cart['price']): ?>
                                ...
                                <div class="text-zinc-600 text-center py-10 font-yekanBakhRegular flex justify-center items-center gap-x-1">
                                  <img src="./../../assets/user/image/icons/cancel.svg" alt="">
                                  سبد خرید خالی است !
                                </div>
                                <?php
                            }
                            ...
                            $getOneProduct = getOneProduct($item['product_id']);
                                $images = json_decode($getOneProduct['images'], true);
                                $mainImage = $getOneProduct['main_image'] ?? ($images[0] ?? '');
                                $thumbnailProductCart = !empty($mainImage) ? "public/images/products/{$mainImage}" : '';
                            ?>
                                <li id="product<?= $getOneProduct['id'] ?>">
                                    <div class="flex gap-x-2 pt-5">
                                        <!-- Product -->
                                        <div class="relative min-w-fit">
                                            <a href="/singleProduct?slug=<?= $getOneProduct['slug'] ?>&code=<?= $getOneProduct['id']?>">
                                                <img alt="" class="h-[120px] w-[120px]" src="<?= $thumbnailProductCart ? "../../" . $thumbnailProductCart : '' ?>">
                                            </a>
                                        </div>
                                        <div class="w-full flex gap-x-1">
                                            <!-- Title -->
                                            <a class="line-clamp-2 h-12 text-zinc-700" href="/singleProduct?slug=<?= $getOneProduct['slug'] ?>&code=<?= $getOneProduct['id']?>">
                                                <?= $getOneProduct['title']?>
                                            </a>
                                            <!-- delete -->
                                            <div class="size-8 group cursor-pointer rounded-lg p-1 bg-primary-400 hover:bg-primary-500 transition">
                                    ...
                                    <div class="flex justify-between px-6 w-full pb-2">
                                        <!-- color -->
                                        <?php if (!empty($item['color']) && !empty($item['titleColor'])): ?>
                                            <div class="flex gap-x-2 text-sm text-gray-600 items-center">
                                                <div
                                                        class="w-4 h-4 rounded-full border"
                                                        style="background-color: <?= htmlspecialchars($item['color']) ?>;">
                                                </div>
                                                <?= htmlspecialchars($item['titleColor']) ?>
                                            </div>
                                        <?php endif; ?>
                                        <!-- Price -->
                                        <div class="flex flex-col items-end">
                    ...
                    <div class="flex items-center justify-between border-t border-gray-100 p-5">
                        <div class="flex flex-col items-center gap-y-1">
                            ...
                            <div class="text-zinc-600">
                                <span class="font-bold" id="priceCart"><?= sumCart() ?></span>
