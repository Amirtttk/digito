<main class="mt-0 md:mt-8">
      ...
      <!-- main blog -->
        ...
        if ($getAllBlogTop){
            foreach ($getAllBlogTop as $AllBlogTop){
                if ($AllBlogTop['image']){
                    $thumbnail1 = str_replace(PATH_UPLOADS_DIR, 'public/', $AllBlogTop['image']);
                }
                ...
                ?>
                            ...
                            <div class="card swiper-slide bg-white rounded-3xl border hover:border-zinc-300 transition border-zinc-300 group p-2 md:p-3 hover:drop-shadow-lg">
                                <a href="/blogSingle?trak=<?= $AllBlogTop['id'] ?>&slug=<?= $AllBlogTop['slug'] ?>" class="image-box block overflow-hidden rounded-lg md:rounded-2xl">
                                    <img class="rounded-2xl w-full transition-transform duration-300 ease-in-out group-hover:rotate-3 group-hover:scale-110 lazyload" src="<?= $thumbnail1 ? "../../" . $thumbnail1 : '' ?>" alt="blog"/>
                                </a>
                                <div class="p-2 mt-2">
    ...
    <div class="flex flex-col lg:flex-row px-4 md:px-20 mt-10 gap-5 pb-20">
      <!-- category -->
        <div class="lg:w-3/12 border border-zinc-300 bg-white h-fit rounded-3xl p-3 space-y-2">
            <a onclick="selectDataInBlog(null)" class="flex justify-between cursor-pointer w-full py-3 px-4 hover:bg-gray-100 rounded-xl">
                <div class="flex items-center gap-x-1">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path class="stroke-zinc-800" d="M15.25 2.75H8.75C7.09315 2.75 5.75 4.09315 5.75 5.75V18.25C5.75 19.9069 7.09315 21.25 8.75 21.25H15.25C16.9069 21.25 18.25 19.9069 18.25 18.25V5.75C18.25 4.09315 16.9069 2.75 15.25 2.75Z" stroke="#71717b" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                        <path class="stroke-zinc-800" d="M11 17.75H13" stroke="" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                    </svg>
                    <span class="text-zinc-800">جدید ترین ها</span>
                </div>
            </a>
            <?php
            ...
            if ($getAllBlogCategories){
                foreach ($getAllBlogCategories as $AllBlogCategories){
                    ?>
                    <a onclick="selectDataInBlog(<?= $AllBlogCategories['id'] ?>)" class="flex justify-between cursor-pointer w-full py-3 px-4 hover:bg-gray-100 rounded-xl">
                        <div class="flex items-center gap-x-1">
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path class="stroke-zinc-800" d="M15.25 2.75H8.75C7.09315 2.75 5.75 4.09315 5.75 5.75V18.25C5.75 19.9069 7.09315 21.25 8.75 21.25H15.25C16.9069 21.25 18.25 19.9069 18.25 18.25V5.75C18.25 4.09315 16.9069 2.75 15.25 2.75Z" stroke="#71717b" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                                <path class="stroke-zinc-800" d="M11 17.75H13" stroke="" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                            </svg>
                            <span class="text-zinc-800"><?= $AllBlogCategories['title']?></span>
                        </div>
                    </a>
                    <?php
                }
        ...
        if ($getAllBlog){
            ?>
            <div id="selectData" class="lg:w-9/12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                <?php
                foreach ($getAllBlog as $AllBlog){
                    if ($AllBlog['image']){
                        $thumbnail = str_replace(PATH_UPLOADS_DIR, 'public/', $AllBlog['image']);
                    }
                    $date = jdate("r", (dateToTimestamp($AllBlog['createAt'])));
                    $createAt1 = $date ;

                    ?>
                    <div class="bg-white rounded-3xl border hover:border-zinc-300 transition border-zinc-300 h-fit group p-2 md:p-3 hover:drop-shadow-lg">
                        <a href="/blogSingle?trak=<?= $AllBlog['id'] ?>&slug=<?= $AllBlog['slug'] ?>" class="image-box block overflow-hidden rounded-lg md:rounded-2xl">
                            <img class="rounded-2xl w-full" src="<?= $thumbnail ? "../../" . $thumbnail : '' ?>" alt="blog"/>
                        </a>
                        <div class="p-2 mt-2">
                            <a href="/blogSingle?trak=<?= $AllBlog['id'] ?>&slug=<?= $AllBlog['slug'] ?>" class="text-xs md:text-sm font-normal md:font-semibold h-8 lg:h-10 line-clamp-2 text-zinc-700 text-right">
                                <?= $AllBlog['title'] ?>
                            </a>
                            <div class="flex justify-between mt-8">
                                <div class="text-xs flex gap-x-1 items-center text-zinc-400">
                                ...
                                <a href="/blogSingle?trak=<?= $AllBlog['id'] ?>&slug=<?= $AllBlog['slug'] ?>" class="flex justify-center items-center gap-x-1 group w-fit text-xs md:text-sm transition text-zinc-600 group-hover:text-zinc-700">
                                    ادامه مطلب
                                    <svg
                                            class="fill-zinc-600 transition group-hover:fill-zinc-700 size-2 md:size-4"
                                            xmlns="http://www.w3.org/2000/svg"
