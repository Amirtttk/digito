    ...
    <nav class="flex items-center gap-1 md:gap-5 w-full md:w-10/12 md:mx-auto">
      <a class="flex flex-col md:flex-row items-center w-fit min-w-24 md:min-w-32 gap-1 text-primary-500 text-sm" href="./cart.html">
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <path class="stroke-primary-500" d="M15.0742 8.8568C14.9303 8.87118 14.8255 9.10815 14.7562 9.22021C14.6354 9.41529 14.5162 9.57762 14.3665 9.75099C14.0074 10.1667 13.4839 10.4894 13.0467 10.8173C12.7359 11.0504 12.3724 11.3453 12.0162 11.5011C11.6944 11.6419 11.3865 11.1364 11.244 10.9225" stroke="black" stroke-width="3" stroke-linecap="round"/>
        </svg>
        سبدخرید
      </a>
      ...
      <a class="flex flex-col md:flex-row items-center w-fit min-w-24 md:min-w-32 gap-1 text-zinc-700 text-sm" href="./cart.html">
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path class="fill-zinc-700" d="M12 10C14.2091 10 16 8.20914 16 6C16 3.79086 14.2091 2 12 2C9.79086 2 8 3.79086 8 6C8 8.20914 9.79086 10 12 10Z" fill="black"/>
          <path class="fill-zinc-700" d="M20 17.5C20 19.985 20 22 12 22C4 22 4 19.985 4 17.5C4 15.015 7.582 13 12 13C16.418 13 20 15.015 20 17.5Z" fill="black"/>
        </svg>
        اطلاعات پرداخت
      </a>
      <div class="h-0.5 w-full bg-gradient-to-r from-white via-zinc-300 to-white"></div>
    ...
    <div class="flex flex-col lg:flex-row px-4 md:px-20 mt-10 gap-5 pb-20">
      <!-- <div class="flex flex-col justify-center items-center mx-auto">
        <div class="text-zinc-600 text-center mx-auto py-10 font-yekanBakhRegular text-xl md:text-3xl flex justify-center items-center gap-x-1">
          <img class="size-6 md:size-10" src="./assets/image/icons/cancel.svg" alt="">
          سبد خرید خالی است !
        </div>
        <a href="" class="group/edit bg-primary-500 hover:bg-primary-400 text-white px-5 py-3 rounded-lg shadow-lg transition-all">
          مشاهده محصولات پرتخفیف
        </a>
      </div> -->
      <div id="parentAllDivCart" class="lg:w-9/12 border border-zinc-200 rounded-xl divide-y divide-zinc-200">
            <?php
            ...
            $getOneProduct = getOneProduct($cart['product_id']);
            $mainImage = $getOneProduct['main_image'] ?? ($images[0] ?? '');
            if ($getOneProduct['main_image']){
                $thumbnail = !empty($mainImage) ? "public/images/products/{$mainImage}" : '';
            }
                ?>
                <div id="parentDivProductInPageCart<?= $getOneProduct['id'] ?>" class="mt-7 flex flex-col md:flex-row gap-y-5 p-4">
                    ...
                    <div class="mr-2 md:mr-5 w-full flex flex-wrap gap-y-3">
                        <div class="w-full h-fit md:w-9/12">
                            <a href="" class=" text-sm sm:text-base text-zinc-700 mb-5">
                                <?= $cart['title'] ?>
                            </a>
                            <?php
                            if($cart['variant_id']){
                                ?>
                                <div class="text-zinc-700 flex items-center gap-x-1 mt-5">
                                    <div class="size-4 rounded-full" style="background-color: <?= $cart['color'] ?>;">
                                    </div>
                                    <div class="text-xs">
                                        <?= $cart['titleColor'] ?>
                                    </div>
                            ...
                            <div class="text-gray-700">
                                <div class="flex gap-x-2 items-center">
                                    <div class="quantity-container flex mr-auto h-10 w-24 items-center justify-between rounded-lg border border-gray-100 px-2 py-1">
                                            ...
                                            <!-- + -->
                                            <button
                                                    class="btn-inc cursor-pointer"
                                                    data-product="<?= $cart['product_id'] ?>"
                                                    data-variant="<?= $cart['variant_id'] ?>"
                                                    onclick="QuantityCart(<?= $cart['product_id'] ?>,'<?= $cart['variant_id'] ?>','yes')"
                                                    type="button">
                                            ...
                                            <!-- count -->
                                            <input
                                                    id="countItem_<?= $cart['product_id'] ?>_<?= $cart['variant_id'] ?>"
                                                    type="number"
                                            ...
                                            <!-- - -->
                                            <button
                                                    class="btn-dec cursor-pointer"
                                                    data-product="<?= $cart['product_id'] ?>"
                                                    data-variant="<?= $cart['variant_id'] ?>"
                                                    onclick="QuantityCart(<?= $cart['product_id'] ?>,'<?= $cart['variant_id'] ?>','no')"
                                                    type="button">
                                    ...
                                    <button  onclick="deleteFromCart(<?= $cart['product_id'] ?>,'<?= $cart['variant_id'] ?>')" class="cursor-pointer bg-white px-2.5 py-2 rounded-lg border border-gray-100" type="button" data-action="decrement">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                            <path fill-rule="evenodd" clip-rule="evenodd" d="M15.7628 9H7.63719C7.18864 9 6.82501 9.37295 6.82501 9.833V16.5C6.82501 17.8807 7.91632 19 9.26251 19H14.1375C14.784 19 15.404 18.7366 15.8611 18.2678C16.3182 17.7989 16.575 17.163 16.575 16.5V9.833C16.575 9.37295 16.2114 9 15.7628 9Z" stroke="red" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                                            <path fill-rule="evenodd" clip-rule="evenodd" d="M14.625 7L13.9191 5.553C13.7541 5.21427 13.4167 5.0002 13.0475 5H10.3526C9.98338 5.0002 9.64596 5.21427 9.48092 5.553L8.77502 7H14.625Z" stroke="red" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                                            <path d="M10.8247 12.333C10.8247 11.9188 10.4889 11.583 10.0747 11.583C9.66047 11.583 9.32469 11.9188 9.32469 12.333H10.8247ZM9.32469 15.666C9.32469 16.0802 9.66047 16.416 10.0747 16.416C10.4889 16.416 10.8247 16.0802 10.8247 15.666H9.32469ZM14.0753 12.333C14.0753 11.9188 13.7396 11.583 13.3253 11.583C12.9111 11.583 12.5753 11.9188 12.5753 12.333H14.0753ZM12.5753 15.666C12.5753 16.0802 12.9111 16.416 13.3253 16.416C13.7396 16.416 14.0753 16.0802 14.0753 15.666H12.5753ZM14.625 6.25C14.2108 6.25 13.875 6.58579 13.875 7C13.875 7.41421 14.2108 7.75 14.625 7.75V6.25ZM16.575 7.75C16.9892 7.75 17.325 7.41421 17.325 7C17.325 6.58579 16.9892 6.25 16.575 6.25V7.75ZM8.77501 7.75C9.18923 7.75 9.52501 7.41421 9.52501 7C9.52501 6.58579 9.18923 6.25 8.77501 6.25V7.75ZM6.82501 6.25C6.4108 6.25 6.07501 6.58579 6.07501 7C6.07501 7.41421 6.4108 7.75 6.82501 7.75V6.25ZM9.32469 12.333V15.666H10.8247V12.333H9.32469ZM12.5753 12.333V15.666H14.0753V12.333H12.5753ZM14.625 7.75H16.575V6.25H14.625V7.75ZM8.77501 6.25H6.82501V7.75H8.77501V6.25Z" fill="red"></path>
                                        </svg>
                                    </button>
                                </div>
                                <?php
                                $price    = (float)$cart['price'];
                                $discount = (float)($cart['discount'] ?? $cart['price']);
                                ?>
            ...
            ?>
                <div class="flex flex-col justify-center items-center mx-auto">
                    <div class="text-zinc-600 text-center mx-auto py-10 font-yekanBakhRegular text-xl md:text-3xl flex justify-center items-center gap-x-1">
                      <img class="size-6 md:size-10" src="./../../assets/user/image/icons/cancel.svg" alt="">
                      سبد خرید خالی است !
                    </div>
                    <a href="" class="group/edit bg-primary-500 hover:bg-primary-400 text-white px-5 py-3 rounded-lg shadow-lg transition-all">
                      مشاهده محصولات پرتخفیف
          ...
          <?php
            ...
            if (isset($_SESSION['cart']) && count($_SESSION['cart']) > 0) {
                foreach ($_SESSION['cart'] as $cart) {
                    ...
                    $priceData = json_decode($getOneProduct['price'], true);
                    ?>
                  <div id="parentDivProductInPageCart<?= $getOneProduct['id'] , $cart['variant_id'] ?>" class="mt-7 flex flex-col md:flex-row gap-y-5 p-4">
                  <div class="w-10/12 mx-auto max-w-52 md:max-w-36">
                        <img src="<?= $thumbnail ? "../../" . $thumbnail : '' ?>" alt="">
                    </div>
                    <div class="mr-2 md:mr-5 w-full flex flex-wrap gap-y-3">
                        ...
                        <div class="w-full md:w-3/12 flex justify-end">
                            <div class="text-gray-700">
                                <div class="flex gap-x-2 items-center">
                                    <div class="quantity-container flex mr-auto h-10 w-24 items-center justify-between rounded-lg border border-gray-100 px-2 py-1">
                                        <button  data-product="<?= $cart['product_id'] ?>"
                                                 data-variant="<?= $cart['variant_id'] ?>" onclick="QuantityCart(<?= $cart['product_id'] ?>,'<?= $cart['variant_id'] ?>','yes')" class="cursor-pointer" type="button" data-action="increment">
                                        ...
                                        <input  id="countItem_<?= $cart['product_id'] ?>_<?= $cart['variant_id'] ?>"
                                                type="number"
                                                value="<?= (int)$cart['quantity'] ?>"
                                                disabled
                                                class="flex h-5 w-full grow select-none items-center justify-center bg-transparent text-center text-sm text-zinc-700 outline-none"
                                        />
                                        <button  data-product="<?= $cart['product_id'] ?>"
                                                 data-variant="<?= $cart['variant_id'] ?>" onclick="QuantityCart(<?= $cart['product_id'] ?>,'<?= $cart['variant_id'] ?>','no')" class="cursor-pointer" type="button" data-action="decrement">
      ...
      <div class="lg:w-3/12 lg:pt-5 my-10 lg:my-0">
        <div class="flex justify-between items-center text-primary-500">
          ...
          <div class="flex">
            <div id="sumPriceInPageCart" class="text-2xl md:text-4xl font-yekanBakhBold"><?= sumCart() ?></div>
        ...
        <div class="flex justify-between items-center text-zinc-600 mt-5">
          ...
          <div class="flex">
            <div id="sumPriceInPageCart1" class="md:text-xl"><?= sumCart() ?></div>
          ...
          if (isset($_SESSION['user_sending'])) {
              ?>
              <a onclick="payment()" class="block bg-primary-500 hover:bg-primary-400 text-white text-center mt-10 px-5 md:px-2.5 py-3 md:py-4 rounded-xl shadow-lg transition-all font-yekanBakhBold md:text-lg">
                  ثبت و ادامه
              </a>
              <?php
          }else{
              ?>
              <a onclick="payment(false)" class="block bg-primary-500 hover:bg-primary-400 text-white text-center mt-10 px-5 md:px-2.5 py-3 md:py-4 rounded-xl shadow-lg transition-all font-yekanBakhBold md:text-lg">
                  ثبت و ادامه
              </a>
              <?php
          }
