<?php
$getOneBlogForSlug = getOneBlogForSlug(GET('slug'),GET('trak'));
$getOneBlogCategories = getOneBlogCategories($getOneBlogForSlug['blog_categories_id']);
if ($getOneBlogForSlug)
    if ($getOneBlogForSlug['image']){
        $thumbnail = str_replace(PATH_UPLOADS_DIR, 'public/', $getOneBlogForSlug['image']);
    }

$date = jdate("r", (dateToTimestamp($getOneBlogForSlug['createAt'])));
$createAt = $date;
?>
<main class="mt-0 md:mt-8">
    <div class="flex flex-col lg:flex-row px-4 md:px-10 gap-5 pb-20">
        <?php
        $getRelatedBlog = getRelatedBlog($getOneBlogForSlug['blog_categories_id'],GET('trak'));
        if ($getRelatedBlog){
        ?>
      <div class="lg:w-3/12 h-fit  order-2 md:order-1">
        ...
        <div class="border border-zinc-300 rounded-3xl p-3 space-y-8">
            ...
            if ($RelatedBlog['image']){
                $thumbnail1 = str_replace(PATH_UPLOADS_DIR, 'public/', $RelatedBlog['image']);
            }
            $date = jdate("r", (dateToTimestamp($getOneBlogForSlug['createAt'])));
            $createAt1 = $date;
            ?>
          <div class="transition group">
            <a href="/blogSingle?trak=<?= $RelatedBlog['id'] ?>&slug=<?= $RelatedBlog['slug'] ?>" class="image-box block overflow-hidden rounded-lg md:rounded-2xl">
            ...
            <div class="p-2 mt-2">
              <a href="/blogSingle?trak=<?= $RelatedBlog['id'] ?>&slug=<?= $RelatedBlog['slug'] ?>" class="text-xs md:text-sm font-normal md:font-semibold h-8 lg:h-10 line-clamp-2 text-zinc-700">
                  <?= $RelatedBlog['title'] ?>
              </a>
              <div class="flex justify-between mt-8">
                ...
                <a href="/blogSingle?trak=<?= $RelatedBlog['id'] ?>&slug=<?= $RelatedBlog['slug'] ?>" class="flex justify-center items-center gap-x-1 group w-fit text-xs md:text-sm transition text-zinc-600 group-hover:text-zinc-700">
                  ادامه مطلب
                  <svg
      ...
      <div class="lg:w-9/12 order-1 md:order-2">
        <div class="mb-7 text-xs flex flex-wrap space-y-2 md:space-y-0 items-center gap-x-2 opacity-90">
          ...
          <a class="text-zinc-500 hover:text-primary-500 transition">
              <?= $getOneBlogCategories['title'] ?>
          </a>
          <svg class="size-3 fill-zinc-500" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#3d3d3d" viewBox="0 0 256 256"><path d="M165.66,202.34a8,8,0,0,1-11.32,11.32l-80-80a8,8,0,0,1,0-11.32l80-80a8,8,0,0,1,11.32,11.32L91.31,128Z"></path></svg>
          <a class="text-primary-500" href="">
              <?= $getOneBlogForSlug['title'] ?>
          </a>
        </div>
        <div class="border border-zinc-300 rounded-3xl p-1.5 md:p-3">
          <img class="w-full h-72 md:h-96 rounded-3xl object-cover object-center mb-3.5" src="<?= $thumbnail1 ? "../../" . $thumbnail1 : '' ?>" alt="blog">
          <div class="flex gap-4 mb-10 flex-wrap">
            ...
            <div class="text-xs flex gap-x-1 items-center text-zinc-400">
              <svg class="fill-zinc-400" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#000000" viewBox="0 0 256 256"><path d="M128,24A104,104,0,1,0,232,128,104.11,104.11,0,0,0,128,24ZM74.08,197.5a64,64,0,0,1,107.84,0,87.83,87.83,0,0,1-107.84,0ZM96,120a32,32,0,1,1,32,32A32,32,0,0,1,96,120Zm97.76,66.41a79.66,79.66,0,0,0-36.06-28.75,48,48,0,1,0-59.4,0,79.66,79.66,0,0,0-36.06,28.75,88,88,0,1,1,131.52,0Z"></path></svg>
                نوشته شده توسط <?= $getOneBlogForSlug['author'] ?>
            </div>
            <div class="text-xs flex gap-x-1 items-center text-zinc-400">
          ...
          <p class="text-xl md:text-3xl text-zinc-800 font-yekanBakhBold my-6 text-center">
              <?= $getOneBlogForSlug['title'] ?>
          </p>
            <?= $getOneBlogForSlug['description'] ?>
        </div>
      </div>
    </div>
  </main>
<?php
$pageTitle = "جزییات مقاله";
$pageScript = "
