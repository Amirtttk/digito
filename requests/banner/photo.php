...
if (!empty($_FILES['image']['size'])) {
    ...
    $path = PATH_UPLOADS_DIR . 'images/banner/' . $new_name;
    if (move_uploaded_file($_FILES['image']['tmp_name'], $path)) {
        $getOneCateqory = getOneBanner(POST('id'));
        $table = 'banner';
        $fields = [
            'image' => $path,
            'image_name' => $new_name,
        ...
        $oldImage = 'no';
        if ($updatePhoto) {
            $dir = PATH_UPLOADS_DIR . 'images/banner/' . $getOneCateqory['image_name'];
            if (file_exists($dir)) {
                unlink($dir);
            }
            $oldImage = 'yes';
            responseJson([
                ...
                'type' => 'success',
                'status' => 200,
                'src' => 'public/images/banner/' . $new_name,
                'oldImage' => $oldImage
            ]);
        } else {
            responseJson([
