...
foreach ($files as $key => $file) {
    ...
    $path = PATH_UPLOADS_DIR . 'images/aboutUs/' . $new_name;
    if (move_uploaded_file($_FILES['image']['tmp_name'], $path)) {
        $getOneCateqory = getAllaAboutUs();
        $table = 'aboutus';
        $filds = [
            'image' => PATH_UPLOADS_DIR . "images/aboutUs/" . $new_name,
            'image_name' => $new_name,
        ];
        $updatePhoto = updateRecordToDatabase($table, $filds, POST('id'),'id');
        $oldImage = 'no';
        if ($updatePhoto) {
            $dir = PATH_UPLOADS_DIR . 'images/aboutUs/' . $getOneCateqory['image_name'];
            if (file_exists($dir)) {
                unlink($dir);
            }
            $oldImage = 'yes';
            responseJson([
                'text' => 'تصویر صفحه اصلی با موفقیت ویرایش شد',
                'type' => 'success',
                'status' => 200,
                'src' => PATH_UPLOADS_DIR . "images/aboutUs/" . $new_name,
                'oldImage' => $oldImage
            ]);
        } else {
            responseJson([
