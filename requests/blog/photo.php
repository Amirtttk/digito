...
if (!empty($_FILES['image']['size'])) {
    ...
    $path = PATH_UPLOADS_DIR . 'images/blog/' . $new_name;
    if (move_uploaded_file($_FILES['image']['tmp_name'], $path)) {
        $getOneCateqory = getOneBlog(POST('id'));
        $table = 'blog';
        $fields = [
            'image' => $path,
            'image_name' => $new_name,
        ...
        $oldImage = 'no';
        if ($updatePhoto) {
            $dir = PATH_UPLOADS_DIR . 'images/blog/' . $getOneCateqory['image_name'];
            if (file_exists($dir)) {
                unlink($dir);
            }
            $oldImage = 'yes';
            responseJson([
                'text' => 'تصویر مقاله با موفقیت ویرایش شد',
                'type' => 'success',
                'status' => 200,
                'src' => 'public/images/blog/' . $new_name,
                'oldImage' => $oldImage
            ]);
        } else {
            responseJson([
